...
router.post('/generateReport',auth.authenticateToken,(req, res) => { 
    ...
    var prodcutDetailsReport = JSON.stringify(orderDetails.productDetails);
    ...
    connection.query(query, [orderDetails.name,generatedUuid,orderDetails.email,orderDetails.contactNumber,orderDetails.paymentMethod,orderDetails.totalAmount,orderDetails.productDetails,res.locals.email],(err,results) => {
        if (!err) {
            ejs.renderFile(path.join(__dirname,'', "report.ejs"), 
                {
                    productDetails:prodcutDetailsReport, name:orderDetails.name,email:orderDetails.email,contactNumber:orderDetails.contactNumber,paymentMethod:orderDetails.paymentMethod,totalAmount:orderDetails.totalAmount},(err, results) => {
                    if (err) {
        ...
        ejs.renderFile(path.join(__dirname, '', "report.ejs"),
                {
                    ...
                    email:orderDetails.email,contactNumber:orderDetails.contactNumber,
